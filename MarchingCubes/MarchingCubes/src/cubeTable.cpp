...
char* CubeTable::getVertexArray(int idx){
	return &table3D[2*idx][1];
}
char* CubeTable::getIndexArray(int idx){
	return &table3D[2*idx+1][1];
}
...
void CubeTable::genTable3D(){
	...
	for(int stateIdxOfCube=0; stateIdxOfCube<256; stateIdxOfCube++){
		...
		cout<<"vertsByCube: ("<<int(table3D[2*stateIdxOfCube][0])<<") ";
		for(int j=0;j<table3D[2*stateIdxOfCube][0];j++) cout<<(vertsByCube[j]>>2)<<"-"<<((vertsByCube[j]>>2)|(1<<(vertsByCube[j]&3)))<<" ";
		cout<<endl<<"indicesByCube: ("<<int(table3D[2*stateIdxOfCube+1][0])<<") ";
		for(int j=0;j<table3D[2*stateIdxOfCube+1][0];j++) cout<<int(indicesByCube[j])<<" ";
	}
	cout<<endl<<"all cases taken care of!"<<endl;
	cout<<"LOOKUP TABLE GENERATION COMPLETE"<<endl;
	cout<<"@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"<<endl;
}

// diagnostic printout
void CubeTable::diag(int idx){
	cout<<"case "<<idx<<"\t";
	cout<<"vertsByCube: ("<<int(table3D[2*idx][0])<<") ";
	for(int j=0;j<table3D[2*idx][0];j++) cout<<(table3D[2*idx][j+1]>>2)<<"-"<<((table3D[2*idx][j+1]>>2)|(1<<(table3D[2*idx][j+1]&3)))<<" ";
	cout<<endl<<"\tindicesByCube: ("<<int(table3D[2*idx+1][0])<<") ";
	for(int j=0;j<table3D[2*idx+1][0];j++) cout<<int(table3D[2*idx+1][j+1])<<" ";
	cout<<endl;
}
